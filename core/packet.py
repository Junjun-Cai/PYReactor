...
class BasePacket :
    ...
    def CopyFromListBuf(self, list_buf):
        ...
        if(len(list_buf) > const.BY_MAX_PACKET_LEN):
            ...
            self.m_protocol = ProtocolParser.ParseProtocol(list_buf)
...
class OutPacket ( BasePacket):
    ...
    def Begin(self, cmd, cversion = 0x1, protocol = g_proctol):
        self.m_protocol = protocol
        head = ProtocolParser.GetHeadObj(protocol)
...
class CPacketParser:
    ...
    def ParsePacket(self, list_buf):
        ...
        proctol = ProtocolParser.ParseProtocol(list_buf)
